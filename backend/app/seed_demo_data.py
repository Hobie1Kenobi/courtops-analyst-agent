...
def create_cases(db: Session, months: int = 6) -> None:
    ...
    for i in range(600):
        ...
        case = Case(
            case_number=f"MC-{filing_date.year}-{i:05d}",
            defendant_name=f"Defendant {i}",
            charge_type=random.choice(["Speeding", "Parking", "Expired Registration", "Failure to Appear"]),
            status=status,
            ...
            fine_amount=random.choice([100.0, 150.0, 250.0, 300.0]),
            amount_paid=random.choice([0.0, 50.0, 100.0, 150.0, 250.0, 300.0]),
        )
        db.add(case)
    db.commit()
